...
(function($){
    ...
    h.append("<style type='text/css'>  .JColResizer{table-layout:fixed;}   .JCLRgrips{ height:0px; position:relative;} .JCLRgrip{margin-left:-5px; position:absolute; z-index:5; } .JCLRgrip .JColResizer{position:absolute;background-color:red;filter:alpha(opacity=1);opacity:0;width:10px;height:100%;cursor: e-resize;top:0px} .JCLRLastGrip{position:absolute; width:1px; } .JCLRgripDrag{ border-left:1px dotted black;  } .JCLRFlex{width:auto!important;} .JCLRgrip.JCLRdisabledGrip .JColResizer{cursor:default; display:none;}.JCLRgrip.JCLRdisabledGrip .grip{cursor:default; display:none;}</style>");
    ...
    var createGrips = function(t){
        ...
        th.each(function(i){                        //iterate through the table column headers
            ...
            var gripDiv = $('<div class="JCLRgrip"></div>');
            var g = $(t.gc.append(gripDiv)[0].lastChild); //add the visual node to be used as grip
            ...
            if (typeof (t.opt.useDatableId) !== "undefined"){
                gripDiv.bind('dblclick', onDblclickColshow); //bind the mousedown event to start dragging
            }
            if (!dc){
        ...
        //不拖动的不显示竖线
        if (typeof (t.opt.useDatableId) !== "undefined") {
            for(var i=0;i<t.dc.length;i++) {
                $("#"+t.opt.useDatableId).find("thead th").eq(t.dc[i]).css("border-right","0")
            }
        }
        
    };
    //一列中内容最长的宽
    var GetLength = function(coldata) {
        var maxlen = 0;
        var hideColCon = "";
        if($(".hideColCon").length <= 0) {
            hideColCon = document.createElement("span");
            hideColCon.style.visibility = "hidden";
            hideColCon.style.display = "inline-block";
            hideColCon.style.position = "fixed";
            hideColCon.style.top = 0;
            document.body.appendChild(hideColCon);
            hideColCon.className="hideColCon";
        } else {
            hideColCon = document.getElementsByClassName("hideColCon")[0];
        }
        for (var i =0; i < coldata.length; i++) {
            if (coldata[i] == " ") {
                hideColCon.innerHTML = "&nbsp;";
            } else {
                hideColCon.innerHTML = coldata[i];
                var colwidth = parseInt($(hideColCon).css("width").slice(0,-2))+10+16;
                if(maxlen < colwidth) {
                    maxlen = colwidth;
                }
            }
    ...
    //重写表头宽
    var tableResize = function(widthArr,table){
        for(var t in tables){
            if( tables.hasOwnProperty( t ) && t == table) {
                t = tables[t];
                var i, mw=0;
    ...
    //双击表头根据内容长度全部展示
    var onDblclickColshow = function(){
        //datatableName,tableId,index,wrap
        var o = $(this).data(SIGNATURE);            //retrieve grip's data
        var t = tables[o.t],  g = t.g[o.i];         //shortcuts for the table and grip objects
        var datatableName = $("#" + t.opt.useDatableId).DataTable();
        var tableId = $("#" + t.opt.useDatableId);
        var index = $(this).index();
        if(t.opt.wrapClass) {
            var wrap = $("." + t.opt.wrapClass);
        } else {
            var wrap = $("#" + t.opt.useDatableId + "_wrapper");
        }
        var $col = datatableName.columns(index);
        var $coldata = $col.data()[0];
        var $maxlen = 0;
        if($coldata.length > 0) {
            $maxlen = GetLength($coldata);
            var $colhead = tableId.find("tr:first th").eq(index);
            $colhead.css("width",$maxlen);

            var widthArr=[];
            var $th=tableId.find("tr:first th");
            for(var i=0;i<$th.length;i++){
                widthArr.push($th.eq(i).css("width"));
                wrap.css("overflow-x","auto");
            }
            tableResize(widthArr,t.opt.useDatableId);
        }
    };
    ...
    var onGripDrag = function(e){
        ...
        if(last=1){                                   //if it is the last grip
